...
def render_anomalies_view(df: pd.DataFrame):
    st.subheader("🤖 Machine Learning Anomaly Detection & Model Benchmark")
    ...
    **Isolation Forest**, **One-Class SVM**, **Local Outlier Factor (LOF)**, **DBSCAN**, and an **MLP Autoencoder**.
    ...
    with col_left:
        st.markdown("#### 🎯 Anomaly Score vs Travel Speed Scatter Matrix")
    ...
    with col_right:
        st.markdown("#### 📊 Anomaly Score Histogram Distribution")
    ...
    st.markdown("---")
    st.markdown("### 🔍 High Anomaly Outliers (Score >= 0.70)")
